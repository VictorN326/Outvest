...
const AgentCard = ({id, name, email, avatar,noOfProperties }: AgentCardProp) => {
  ...
  const generateLink = () => {
    if(currentUser.email === email) return '/my-profiles';
    return `/agents/show/${id}`
  }
  return (
    <Box component={Link} to={generateLink()} width="100%" sx={{display: 'flex', flexDirection: {xs: 'column', sm: 'row'},gap: '20px', padding: '20px',
      '&:hover': {boxShadow: '0 22px 45px 2px rgba(176,176,176,0.1'} }}>
        ...
        <Stack direction="column" justifyContent="space-between" flex={1} gap={{xs: 4, sm:2 }}>
          ...
          <Stack direction="row" flexWrap="wrap" justifyContent="space-between" alignItems="center" gap={2}>
            ...
            <InfoBar icon={<Place sx={{color: '#808191'}}/>} name="Los Angeles, California" />
            <InfoBar icon={<Phone sx={{color: '#808191'}}/>} name="714-873-3222" />
